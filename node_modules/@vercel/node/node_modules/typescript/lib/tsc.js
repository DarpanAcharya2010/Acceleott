...
(function (ts) {
    ...
    function isParameterPropertyDeclaration(node, parent) {
        return ts.hasSyntacticModifier(node, 16476) && parent.kind === 173;
    }
    ...
    function getModifiers(node) {
        if (ts.hasSyntacticModifier(node, 126975)) {
    ...
    function getTextOfJSDocComment(comment) {
        return typeof comment === "string" ? comment
            : comment === null || comment === void 0 ? void 0 : comment.map(function (c) { return c.kind === 324 ? c.text : formatJSDocLink(c); }).join("");
    }
    ts.getTextOfJSDocComment = getTextOfJSDocComment;
    function formatJSDocLink(link) {
        var kind = link.kind === 327 ? "link"
            : link.kind === 328 ? "linkcode"
                : "linkplain";
    ...
    function getEffectiveTypeParameterDeclarations(node) {
        ...
        if (ts.isJSDocTypeAlias(node)) {
            ts.Debug.assert(node.parent.kind === 323);
    ...
    function isJSDocPropertyLikeTag(node) {
        return node.kind === 350 || node.kind === 343;
    }
    ...
    function isGeneratedPrivateIdentifier(node) {
        return ts.isPrivateIdentifier(node) && (node.autoGenerateFlags & 7) > 0;
    }
    ...
    function isDeclaration(node) {
        if (node.kind === 165) {
            return (node.parent && node.parent.kind !== 347) || ts.isInJSFile(node);
        }
    ...
    function isJSDocNode(node) {
        return node.kind >= 312 && node.kind <= 350;
    }
    ...
    function isJSDocTag(node) {
        return node.kind >= 330 && node.kind <= 350;
    }
    ...
    function isRestParameter(node) {
        ...
        return node.dotDotDotToken !== undefined || !!type && type.kind === 321;
    }
...
(function (ts) {
    ...
    function getTokenPosOfNode(node, sourceFile, includeJsDoc) {
        ...
        if (node.kind === 351 && node._children.length > 0) {
    ...
    function isAmbientPropertyDeclaration(node) {
        return !!(node.flags & 16777216) || hasSyntacticModifier(node, 2);
    }
    ...
    function getJSDocCommentRanges(node, text) {
        var commentRanges = (node.kind === 166 ||
            node.kind === 165 ||
            node.kind === 215 ||
            node.kind === 216 ||
            node.kind === 214 ||
    ...
    function isPartOfTypeNode(node) {
        ...
        switch (node.kind) {
            ...
            case 79:
                ...
                ts.Debug.assert(node.kind === 79 || node.kind === 163 || node.kind === 208, "'node' was expected to be a qualified name, identifier or property access in 'isPartOfTypeNode'.");
            case 163:
            case 208:
            case 108: {
                ...
                if (179 <= parent.kind && parent.kind <= 202) {
                    return true;
                }
                switch (parent.kind) {
                    case 230:
    ...
    function isVariableDeclarationInVariableStatement(node) {
        return node.parent.kind === 258
            && node.parent.parent.kind === 240;
    }
